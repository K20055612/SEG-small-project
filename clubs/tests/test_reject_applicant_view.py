...
class RejectUserViewTestCase(TestCase):
    """Tests of the reject_user view."""
    ...
    def setUp(self):
        ...
        self.url = reverse('reject_applicant', kwargs={'user_id': self.applicant.id})

    def test_reject_applicant_url(self):
        self.assertEqual(self.url,f'/reject/{self.applicant.id}/')
    ...
    def test_reject_applicant_with_invalid_id(self):
        self.client.login(username=self.user.email, password='Password123')
        before = User.objects.all().filter(is_applicant=True).count()
        url = reverse('reject_applicant', kwargs={'user_id': self.user.id+9999})
        after = User.objects.all().filter(is_applicant=True).count()
