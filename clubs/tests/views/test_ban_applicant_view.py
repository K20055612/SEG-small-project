...
class BanApplicantViewTestCase(TestCase,LogInTester):
    """Tests of the ban_applicant view."""
    ...
    def test_ban_applicant_with_valid_id(self):
    ...
    def test_ban_applcant_as_owner(self):
    ...
    def test_ban_applicant_with_invalid_id(self):
        self.client.login(username=self.user.username, password='Password123')
        self.assertTrue(self._is_logged_in())
        before = Role.objects.all().filter(club=self.club,club_role='APP').count()
        url = reverse('ban_applicant', kwargs={'club_name':self.club.club_name,'user_id': self.applicant.id+9999})
        after = Role.objects.all().filter(club=self.club,club_role='APP').count()
    ...
    def test_ban_applicant_invalid_club(self):
        self.client.login(username=self.user.username, password='Password123')
        self.assertTrue(self._is_logged_in())
        url = reverse('ban_applicant', kwargs={'club_name':'WRONG CLUB','user_id': self.applicant.id})
        response = self.client.get(url, follow=True)
        response_url = reverse('feed')
        self.assertRedirects(response, response_url, status_code=302, target_status_code=200)
        self.assertTemplateUsed(response, 'feed.html')

    def test_ban_applicant_user_does_not_have_permission_is_member(self):
    ...
    def test_ban_applicant_user_does_not_have_permission_is_applicant(self):
    ...
    def test_ban_applicant_user_does_not_have_permission_is_visitor(self):
